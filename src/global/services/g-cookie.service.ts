...
/**
 * Wraper for a CooikeService for easy replace of lib service.
 * @export
 * @class GCookieService
 */
@Injectable()
export class GCookieService {
    ...
    public check(name: string): boolean {
        ...
        let exists = regexp.test(this.document.cookie);
        return exists;
    }
    ...
    public get(name: string): string {
        if (this.check(name)) {
            name = encodeURIComponent(name);
            let regexp = new RegExp('(?:^' + name + '|;\\s*' + name + ')=(.*?)(?:;|$)', 'g');
            let result = regexp.exec(this.document.cookie);
    ...
    public getAll(): any {
        ...
        if (this.document.cookie && this.document.cookie !== '') {
            let split = this.document.cookie.split(';');
            for (let cookie of split) {
                let currCookie = cookie.split('=');
